return {
  ...
  config = function()
    ...
    vim.keymap.set("n", "<leader>gB", function()
      vim.ui.select(vim.fn.systemlist("git branch --format='%(refname:short)'"), { prompt = "Select branch for history:" }, function(branch)
        if branch then vim.cmd("DiffviewFileHistory --range=" .. branch) end
      end)
    end, { desc = "Git branch history" })
    vim.keymap.set("n", "<leader>gC", function()
      vim.ui.input({ prompt = "Commits range (e.g. HEAD~5..HEAD):" }, function(range)
        if range and range ~= "" then vim.cmd("DiffviewFileHistory " .. range) end
      end)
